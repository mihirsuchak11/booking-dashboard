...
const MIX_BLEND_MODE_CLASS: Record<string, string> = {
...
export default function FloatingLines({
  ...
  useEffect(() => {
    ...
    const blendModeClass = MIX_BLEND_MODE_CLASS[mixBlendMode] ?? "mix-blend-screen";
    containerRef.current.classList.remove(...Object.values(MIX_BLEND_MODE_CLASS));
    containerRef.current.classList.add(blendModeClass);
    ...
    const maxDpr = isCoarsePointer ? 1 : 1.5;
    ...
    const boundsRef = {
      current: { left: 0, top: 0, width: 1, height: 1 }
    };
    ...
    const io =
      typeof IntersectionObserver !== "undefined"
        ? new IntersectionObserver(([entry]) => {
            isActive = Boolean(entry?.isIntersecting) && !document.hidden;
            if (!isActive) stop();
            else start();
          })
        : null;
